...
def product_filter(
    ...
    if (len(sizeIds) != 0) or (len(maxPrices) != 0):
        product_q = (
            ...
            .join(
                VariantSizePrice, VariantSizePrice.variantId == ProductVariant.variantId
            )
    ...
    if len(storeIds) != 0:
        product_q = product_q.filter(
            Products.storeId.in_(storeIds), Products.is_deleted == 0
        )
        counter += 1
    if len(brandIds) != 0:
        product_q = product_q.filter(
            Products.brandId.in_(brandIds), Products.is_deleted == 0
        )
        counter += 1
    if len(colorIds) != 0:
    ...
    if len(maxPrices) != 0:
        product_q = product_q.filter(
            VariantSizePrice.price.between(maxPrices[0], maxPrices[1])
        )
        counter += 1
    if len(sizeIds) != 0:
    ...
    final = product_q.filter(
        Products.is_deleted == 0, ProductVariant.is_deleted == 0
    ).all()
    ...
    if len(sizeIds) != 0:
        for i in range(len(final)):
            size_q = (
                db.query(
                    Sizes.size,
                    VariantSizePrice.price,
                    VariantSizePrice.quantity,
                    VariantSizePrice.variantId,
                )
                .join(Sizes, Sizes.sizeId == VariantSizePrice.sizeId)
                .filter(
                    VariantSizePrice.variantId == final[i][1],
                    VariantSizePrice.sizeId.in_(sizeIds),
                )
                .all()
            )

            image_q = (
                db.query(VariantImages.image_url)
            ...
            variant_json = {
                ...
                "brandName": final[i][3],
                "material": final[i][4],
                "categoryName": final[i][5],
                "subcategoryName": final[i][6],
                "gender": final[i][7],
                "storeName": final[i][8],
                "productName": final[i][9],
                "productDescription": final[i][10],
                "spq": sizes,
                "images": images,
            }
    ...
    return {
        ...
        "message": str(len(newOpList)) + " results found",
        "data": newOpList,
    }
